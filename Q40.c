...
// Function to perform monoalphabetic substitution cipher letter frequency attack
void frequency_attack(const char *ciphertext, int top_plaintexts) {
    ...
    for (int i = 0; i < top_plaintexts; i++) {
        ...
        int shift = 'a' - ciphertext_freq[i].character;
        for (int j = 0; ciphertext[j] != '\0'; j++) {
            char plaintext_char;
            if (isalpha(ciphertext[j])) {
                plaintext_char = ((ciphertext[j] - 'a' + shift + ALPHABET_SIZE) % ALPHABET_SIZE) + 'a';
            } else {
                plaintext_char = ciphertext[j]; // Non-alphabetic characters remain unchanged
            }
...
int main() {
    char ciphertext[] = "Yhkuhupun nlaaly vm aol zhnlz jylhkpun jvumpklualea";
